...
{   //Membered function used to enter Patient's details
    ...
    if((strlen(age)>=3&&age[0]>='2')||(!strlen(age))) goto lb11;
    temp=strtol(age,&p,10);
    if(*p)
    {
        lb11:cout<<"Invalid input for age";
        error_show();
        //system("cls");
        check=2;
        goto lb8;
    ...
    if(!(strlen(Number)==10)||(!strlen(Number))) goto lb9;
    temp=strtol(Number,&p,10);
    if(*p)
    {
        lb9:cout<<"Invalid input for Number";
        error_show();
        //system("cls");
        check=4;
        goto lb8;
...
void Medhistory ::gethistory()
    {   //Function to enter Medical history of the patient
        int check(0);
        lb10:cout<<"Enter the observed symptoms"<<endl;
        cin.sync();
        if(check>=2)
            puts(Symptoms);
        else
...
{   //Function to display basic details of the patient
    ...
    cout<<endl<<"Age of the Patient  : "<<getage();
    cout<<endl<<"Phone number        : +91"<<getnum();
    if(getgen()=='m'||getgen()=='M')
        cout<<endl<<"Gender\t\t    : Male";
    else if(getgen()=='f'||getgen()=='F')
        cout<<endl<<"Gender\t\t    : Female";
    else if(getgen()=='o'||getgen()=='O')
        cout<<endl<<"Gender\t\t    : Other";
    cout<<endl<<"Patient's Address   : "<<getadr();
    ...
    switch (ans)
    {
        case '1':char ch;
            do {
                   ...
                   obj.appoint();
                   lb2:cout<<"Do you want to fill the Medical chart?(Y/N)"<<endl;
                   cin>>ch;
                   ...
                   //system("cls");
                   lb3:cout<<"Want to Enter another Patient?(Y/N)"<<endl;
                   cin>>ch;
                   invalid_check=yes_no_check(ch);
                   if(!invalid_check)
                   {
        ...
        case '2':check='\0';
            ...
            if(!(check=='n'))
            {
              ...
              lb5:cout<<endl<<"Enter 0 for menu\n";
              cin>>ch;
              if(ch=='0')
              {
                //system("cls");
        ...
        case '4':check='t';
               ...
               cout<<endl<<obj.getvisittime()<<"at "<<obj.getvisitday();
               int status=0;
               ...
               if(status)
               {
                   cout<<endl<<"Appointment Status            : In time";
               }
               else if (!status)
                cout<<endl<<"Appointment Status            : Overdue";
               }
               cout<<endl<<"Enter 0 for menu\n";
               cin>>ch;
               if(ch=='0')
               {
                   //system("cls");
        ...
        case '5':lb6:cout<<"Are you sure you want to erase all the Entries?(Y/N)\n";
                 cin>>ch;
                 invalid_check=yes_no_check(ch);
                   if(!invalid_check)
                   {
        ...
        case '3':char choice;
                ...
                lb7:cout<<"Number of patients will be displayed on the basis of the attributes selected"<<endl;
                ...
                if(check[0]=='y'||check[0]=='Y')
                    {
                        ...
                        if(strlen(age_min)>=3&&age_min[0]>='2'||(!strlen(age_min))) goto lb13;
                        ...
                        if(strlen(age_max)>=3&&age_max[0]>='2'||(!strlen(age_max))) goto lb12;
                ...
                if(check[4]=='y'||check[4]=='Y')
                {
                    ...
                    if(strlen(d_lower.dd)>=2&&d_lower.dd[0]>='3'&&d_lower.dd[1]>'1'||(!strlen(d_lower.dd))) goto lb14;
                    ...
                    if(strlen(d_lower.mm)>=2&&d_lower.mm[0]>='1'&&d_lower.mm[1]>'2'||(!strlen(d_lower.mm))) goto lb15;
                    ...
                    if(strlen(d_lower.yy)>=5||(!strlen(d_lower.yy))) goto lb16;
                    ...
                    if(strlen(d_upper.dd)>=2&&d_upper.dd[0]>='3'&&d_upper.dd[1]>'1'||(!strlen(d_upper.dd))) goto lb17;
                    ...
                    if(strlen(d_upper.mm)>=2&&d_upper.mm[0]>='1'&&d_upper.mm[1]>'2'||(!strlen(d_upper.mm))) goto lb18;
                    ...
                    if(strlen(d_upper.yy)>=5||(!strlen(d_upper.yy))) goto lb19;
                ...
                while(file.read((char*)&obj,sizeof(obj)))
                {
                    ...
                    a.str("");b.str("");c.str("");
                    if(check[0]=='y'||check[0]=='Y')flag[0]=age_check(obj.getage(),age_max,age_min);
                    else flag[0]=1;
                    if(check[1]=='y'||check[1]=='Y')flag[1]=gender_check(obj.getgen(),sex);
                    else flag[1]=1;
                    if(check[2]=='y'||check[2]=='Y')flag[2]=genetic_check(obj.getmedhistory().getgenetic(),genetic_ill);
                    else flag[2]=1;
                    if(check[3]=='y'||check[3]=='Y')flag[3]=smoke_check(obj.getmedhistory().getsmoke(),smoke);
                    else flag[3]=1;
                    if(check[4]=='y'||check[4]=='Y')flag[4]=date_check(d_app,d_upper,d_lower);
                    else flag[4]=1;
                if(flag[0]&&flag[1]&&flag[2]&&flag[3]&&flag[4])counting++;
                }
                file.close();
...
{   //Function to check genetic illness
    tolower(objgenetic);
    tolower(genetic);
    if(objgenetic==genetic)return 1;
    return 0;
}
